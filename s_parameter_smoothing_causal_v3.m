function s_parameter_smoothing_causal_v3
% Z域实部/虚部物理外推替换 + 无源性强制（SVD缩放）
% 改进版v3：在S域进行样条插值，避免Z矩阵求逆的数值不稳定性
...
for i = 1:Nport
    for j = 1:Nport
        if jumpMask(i,j)
            ...
            % 在对数频率空间进行分段样条插值
            % 使用更稳定的pchip方法而非cubic spline
            try
                S_real_interp = interp1(logf_low, S_real, logf_low, 'pchip');
                S_imag_interp = interp1(logf_low, S_imag, logf_low, 'pchip');
            catch
                % 如果pchip失败，使用linear插值
                S_real_interp = interp1(logf_low, S_real, logf_low, 'linear');
                S_imag_interp = interp1(logf_low, S_imag, logf_low, 'linear');
            end
...
Z0 = 50;
I_mat = eye(Nport);
Z_smooth = zeros(Nf, Nport, Nport);

for k = 1:Nf
    Sk = squeeze(S_smooth(k,:,:));
    
    % 使用线性方程求解而非矩阵求逆，提高数值稳定性
    % (I - S) * Z/Z0 = I + S
    % Z/Z0 = (I - S) \ (I + S)
    ...
    try
        ...
        if cond_num > 1e10
            % 条件数过大，添加正则化
            denom = denom + 1e-8 * I_mat;
        end
        
        % 使用线性求解（数值更稳定）
        Z_norm = denom \ numer;
        Z_smooth(k,:,:) = Z0 * Z_norm;
        
    catch ME
        fprintf('警告：第%d个频点Z转换失败: %s\n', k, ME.message);
        % 保持原始值
        Z_smooth(k,:,:) = Z(k,:,:);
    end
end
...
function writeSnP_simple(filename, freq, S, params)
    ...
    fprintf(fid, '! S-domain spline interpolation with passivity enforcement\n');
